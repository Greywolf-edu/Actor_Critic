...
class Server():
    def __init__(self, nb_state_feature, nb_action, name):
        ...
        zero_net_weights(self.routing_net)
        ...
        self.net = [self.body_net, self.routing_net, self.critic_net]
    ...
    def get_policy(self, state_vector):
        ...
        actor_body_out = self.actor_body(body_out)
        policy = self.routing_net(actor_body_out)
        charging_time = self.timing_net(actor_body_out)
        return policy, charging_time
